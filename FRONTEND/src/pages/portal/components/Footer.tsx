...
type SocialItem = { label: string; href: string; iconKey?: string };
...
const Footer: React.FC<{
  ...
  const quickLinks = useMemo(() => {
    if (Array.isArray(footer?.quickLinks) && footer.quickLinks.length > 0) return footer.quickLinks;
    return defaultQuickLinks;
  }, [footer?.quickLinks]);

  const socials = useMemo(() => {
    if (Array.isArray(footer?.socials) && footer.socials.length > 0) return footer.socials;
    return defaultSocials;
  }, [footer?.socials]);

  const bottomLinks = useMemo(() => {
    if (Array.isArray(footer?.bottomLinks) && footer.bottomLinks.length > 0) return footer.bottomLinks;
  ...
  const badges = useMemo(() => {
    if (Array.isArray(footer?.badges) && footer.badges.length > 0) return footer.badges;
    return [
      ...
      <div className="container mx-auto px-4 md:px-6 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-16 mb-20">
          <div className="space-y-8">
            ...
            <p className="text-slate-500 leading-relaxed max-w-xs">{blurb}</p>
