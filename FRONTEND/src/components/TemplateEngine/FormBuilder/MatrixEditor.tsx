                    ...
                    {config.columns.map((col, idx) => (
                        <div key={idx} className="flex gap-2 group">
                            <input
                                ...
                                className="flex-1 bg-white border rounded-lg p-2 text-xs outline-none focus:ring-1 focus:ring-blue-500"
                            />
                            <button onClick={() => onChange({ ...config, columns: config.columns.filter((_, i) => i !== idx) })} className="text-gray-300 hover:text-red-500 opacity-0 group-hover:opacity-100"><Trash2 size={14} /></button>
                        </div>
                    ))}
                </div>
            </div>
